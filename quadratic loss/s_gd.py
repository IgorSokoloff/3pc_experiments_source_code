...
def gd_estimator(A, x, b, n_workers):
    ...
    grad = grad/n_workers
    return size_value_sent, grad

def gd(x_0, x_star, f_star, A, b, stepsize, eps,  n_workers, experiment_name, project_path, dataset, Nsteps, Nbits):
    ...
    while stopping_criterion(sq_norm_ar[-1], eps, it, Nsteps, bits_od_ar[-1], Nbits):
        x = x - stepsize*g
        size_value_sent, grad = gd_estimator(A, x, b, n_workers)
        g = grad.copy()
        ...
        it += 1
        bits_od_ar.append(it*dim*size_value_sent)
        ...
        if it%PRINT_EVERY ==0:
            ...
            save_data(bits_od_ar, bits_bd_ar, comms_ar, comms_ar, arg_res_ar, func_diff_ar, sq_norm_ar, x.copy(), experiment_name, project_path, dataset)
...
def save_data(bits_od, bits_bd, epochs, comms, arg_res, func_diff, f_grad_norms, x_solution, experiment_name, project_path, dataset):
    print("data saving")
...
experiment_name = "gd_d{0}_nw{1}_s{2}_{3}x".format(dim, n_w, myrepr(s), myrepr(factor))

print ('------------------------------------------------------------------------------------------------')
start_time = time.time()
gd(x_0, x_star, f_star, sX, y, step_size_gd, eps, n_w, experiment_name, project_path, dataset, Nsteps=nsteps, Nbits=nbits)
