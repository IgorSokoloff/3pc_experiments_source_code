"""
3PCv4-Topk-Topk with full gradient
"""
...
def jacqueline_fg_estimator(A, x, b,  k_tk1, k_tk2, g_ar,  n_workers):
    grad = np.zeros(x.shape[0], dtype=np.float64)
    for i in range(n_workers):
        grad_i = quadratic_grad_i(x, A[i], b[i]).copy()
        grad = grad + grad_i
        b_i = g_ar[i] + top_k_compressor(grad_i - g_ar[i], k_tk2)
        g_ar[i] = b_i + top_k_compressor(grad_i - b_i, k_tk1)
    size_value_sent = 64*1e-6
    grad = grad/n_workers
    return g_ar, size_value_sent, grad

def jacqueline_fg(x_0, x_star, f_star, A, b, stepsize, eps, k_tk1, k_tk2, n_workers, experiment_name, project_path, dataset, Nsteps, Nbits):
    currentDT = datetime.datetime.now()
    print(currentDT.strftime("%Y-%m-%d %H:%M:%S"))
    print(experiment_name + f" with Top{k_tk1} and Top{k_tk2} finished")
    ...
    while stopping_criterion(sq_norm_ar[-1], eps, it, Nsteps, bits_od_ar[-1], Nbits):
        x = x - stepsize*g
        g_ar, size_value_sent, grad = jacqueline_fg_estimator(A, x, b, k_tk1, k_tk2, g_ar, n_workers)
        g = np.mean(g_ar, axis=0)
        ...
        it += 1
        bits_od_ar.append(it*(k_tk1+k_tk2)*size_value_sent)
        ...
        if it%PRINT_EVERY ==0:
            ...
            save_data(bits_od_ar, bits_bd_ar, comms_ar, comms_ar, arg_res_ar, func_diff_ar, sq_norm_ar, x.copy(), k_tk1, k_tk2, experiment_name, project_path, dataset)
...
def save_data(bits_od, bits_bd, epochs, comms, arg_res, func_diff, f_grad_norms, x_solution, k_tk1, k_tk2, experiment_name, project_path, dataset):
    print("data saving")
    experiment = '{0}_{1}_{2}'.format(experiment_name, k_tk1, k_tk2)
...
al_tk1 = k_tk1/dim
al_tk2 = k_tk2/dim
al_bar = 1 - (1-al_tk1)*(1-al_tk2)
theta = 1 - np.sqrt(1-al_bar)
beta = (1-al_bar)/theta
Lt = np.sqrt(np.mean(L**2))
step_size_jacqueline_fg = np.float64((1/(L_0 + Lt*np.sqrt(beta/theta)))*factor)

experiment_name = "jacqueline-topk-topk-fg_d{0}_nw{1}_s{2}_{3}x".format(dim, n_w, myrepr(s), myrepr(factor))

print ('------------------------------------------------------------------------------------------------')
start_time = time.time()
jacqueline_fg(x_0, x_star, f_star, sX, y, step_size_jacqueline_fg, eps,  k_tk1, k_tk2, n_w, experiment_name, project_path, dataset, Nsteps=nsteps, Nbits=nbits)
