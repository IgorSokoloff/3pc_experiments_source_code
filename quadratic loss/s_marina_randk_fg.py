"""
MARINA-RandK with full gradient
"""
...
def rand_k_compressor(x, k_rk, rs_rand, probs):
    output = np.zeros(x.shape)
    d = x.shape[0]
    inds = rs_rand.choice(a=np.arange(d), size=k_rk, replace=False, p=probs)
    output[inds] = x[inds]
...
def marina_randk_fg_estimator(A, x, b,  k, prob, g_ar, grads_prev, n_workers, rs, rs_rand, probs):
    ...
    for i in range(n_workers):
        ...
        if c_t==0:
            ...
            g_ar[i] = g_ar[i] + rand_k_compressor(delta_i, k, rs_rand, probs)
            number_of_values_sent = k
        elif c_t==1:
            g_ar[i] = grad_i.copy()
...
def marina_randk_fg(x_0, x_star, f_star, A, b, stepsize, eps, k, prob, n_workers, experiment_name, project_path, dataset, Nsteps, Nbits):
    currentDT = datetime.datetime.now()
    print(currentDT.strftime("%Y-%m-%d %H:%M:%S"))
    print(experiment_name + f" with k={k} has started")
    rs = RandomState(12345)
    rs_rand = RandomState(12346)
    dim = x_0.shape[0]
    ...
    while stopping_criterion(sq_norm_ar[-1], eps, it, Nsteps, bits_od_ar[-1], Nbits):
        x = x - stepsize*g
        g_ar, size_value_sent, number_of_values_sent, grads, grad = marina_randk_fg_estimator(A, x, b, k, prob, g_ar, grads, n_workers, rs, rs_rand,probs)
        g = np.mean(g_ar, axis=0)
        ...
        if it%PRINT_EVERY ==0:
            ...
            save_data(bits_od_ar, bits_bd_ar, comms_ar, comms_ar, arg_res_ar, func_diff_ar, sq_norm_ar, x.copy(), k, experiment_name, project_path, dataset)

    save_data(bits_od_ar, bits_bd_ar, comms_ar, comms_ar, arg_res_ar, func_diff_ar, sq_norm_ar, x.copy(), k, experiment_name, project_path, dataset)
    print(experiment_name + f" with k={k} finished")
    print("End-point:")
    print_last_point_metrics(bits_od_ar, bits_bd_ar, comms_ar, comms_ar, arg_res_ar, func_diff_ar, sq_norm_ar)
...
prob = k_rk/dim
if k_rk == dim:
    step_size_marina_randk_fg = (1/L_0)*factor
else:
    Lt = np.sqrt(np.mean(L**2))
    A_st = (dim/k_rk - 1)/n_w
    M = L_0 + Lt*np.sqrt(((1-prob)/prob)*A_st)
    step_size_marina_randk_fg = np.float64((1/M)*factor)

experiment_name = "marina-randk-fg_d{0}_nw{1}_s{2}_p{4}_{3}x".format(dim, n_w, myrepr(s), myrepr(factor), myrepr(prob))

print ('------------------------------------------------------------------------------------------------')
start_time = time.time()
marina_randk_fg(x_0, x_star, f_star, sX, y, step_size_marina_randk_fg, eps,  k_rk, prob,  n_w, experiment_name, project_path, dataset, Nsteps=nsteps, Nbits=nbits)
