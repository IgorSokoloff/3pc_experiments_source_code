...
def ef21_topk_fg_estimator(A, x, b,  k, g_ar, n_workers):
...
def ef21_topk_fg(x_0, x_star, f_star, A, b, stepsize, eps, k, n_workers, experiment_name, project_path, dataset, Nsteps, Nbits):
    ...
    while stopping_criterion(sq_norm_ar[-1], eps, it, Nsteps, bits_od_ar[-1], Nbits):
        x = x - stepsize*g
        g_ar, size_value_sent, grad = ef21_topk_fg_estimator(A, x, b, k, g_ar, n_workers)
        g = np.mean(g_ar, axis=0)
...
if k_tk == dim:
    step_size_ef21_topk_fg = (1/L_0)*factor
else:
    al = k_tk/dim
    t = -1 + np.sqrt(1/(1-al))
    theta = 1 - (1 - al)*(1 + t)
    ...
    step_size_ef21_topk_fg = np.float64((1/(L_0 + Lt*np.sqrt(beta/theta)))*factor)

experiment_name = "ef21-topk-fg_d{0}_nw{1}_s{2}_{3}x".format(dim, n_w, myrepr(s), myrepr(factor))

print ('------------------------------------------------------------------------------------------------')
start_time = time.time()
ef21_topk_fg(x_0, x_star, f_star, sX, y, step_size_ef21_topk_fg, eps,  k_tk, n_w, experiment_name, project_path, dataset, Nsteps=nsteps, Nbits=nbits)
