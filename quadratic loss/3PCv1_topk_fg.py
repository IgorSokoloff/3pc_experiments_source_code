...
def biased_diana_2_fg_estimator(A, x, b,  k, g_ar, grads_prev, n_workers):
    ...
    for i in range(n_workers):
        ...
        grads[i] = grad_i.copy()
        grad = grad + grad_i
        delta_i = grad_i - grads_prev[i]
        g_ar[i] = grads_prev[i] + top_k_compressor(delta_i, k)
    size_value_sent = 64*1e-6
    grad = grad/n_workers
    return g_ar, size_value_sent, grads, grad

def biased_diana_2_fg(x_0, x_star, f_star, A, b, stepsize, eps, k, n_workers, experiment_name, project_path, dataset, Nsteps, Nbits):
    ...
    while stopping_criterion(sq_norm_ar[-1], eps, it, Nsteps, bits_od_ar[-1], Nbits):
        x = x - stepsize*g
        g_ar, size_value_sent, grads, grad = biased_diana_2_fg_estimator(A, x, b, k, g_ar, grads, n_workers)
        g = np.mean(g_ar, axis=0)
    ...
    save_data(bits_od_ar, bits_bd_ar, comms_ar, comms_ar, arg_res_ar, func_diff_ar, sq_norm_ar, x.copy(), k, experiment_name, project_path, dataset)
    print(experiment_name + f" with k={k_tk} finished")
    print("End-point:")
    print_last_point_metrics(bits_od_ar, bits_bd_ar, comms_ar, comms_ar, arg_res_ar, func_diff_ar, sq_norm_ar)
...
M = L_0 + Lt*np.sqrt(1 - al)
step_size_biased_diana_2_fg = np.float64((1/M)*factor)

experiment_name = "biased-diana-2-topk-fg_d{0}_nw{1}_s{2}_{3}x".format(dim, n_w, myrepr(s), myrepr(factor))

print ('------------------------------------------------------------------------------------------------')
start_time = time.time()
biased_diana_2_fg(x_0, x_star, f_star, sX, y, step_size_biased_diana_2_fg, eps,  k_tk, n_w, experiment_name, project_path, dataset, Nsteps=nsteps, Nbits=nbits)
