...
def marina_topk_fg_estimator(A, x, b,  k, prob, g_ar, grads_prev, n_workers, rs):
    ...
    for i in range(n_workers):
        ...
        if c_t==0:
            ...
            g_ar[i] = g_ar[i] + top_k_compressor(delta_i, k)
...
def marina_topk_fg(x_0, x_star, f_star, A, b, stepsize, eps, k, prob, n_workers, experiment_name, project_path, dataset, Nsteps, Nbits):
    ...
    while stopping_criterion(sq_norm_ar[-1], eps, it, Nsteps, bits_od_ar[-1], Nbits):
        x = x - stepsize*g
        g_ar, size_value_sent, number_of_values_sent, grads, grad = marina_topk_fg_estimator(A, x, b, k, prob, g_ar, grads, n_workers, rs)
        g = np.mean(g_ar, axis=0)
...
prob = k_tk/dim
if k_tk == dim:
    step_size_marina_topk_fg = (1/L_0)*factor
else:
    al = k_tk/dim
    t = -1 + np.sqrt(1/(1-prob))
    theta = 1 - (1 - prob)*(1 + t)
    beta = (1 - prob)*(1 + 1/t)*(1 - al)
    Lt = np.sqrt(np.mean(L**2))
    step_size_marina_topk_fg = np.float64((1/(L_0 + Lt*np.sqrt(beta/theta)))*factor)

experiment_name = "marina-topk-fg_d{0}_nw{1}_s{2}_p{4}_{3}x".format(dim, n_w, myrepr(s), myrepr(factor), myrepr(prob))

print ('------------------------------------------------------------------------------------------------')
start_time = time.time()
marina_topk_fg(x_0, x_star, f_star, sX, y, step_size_marina_topk_fg, eps,  k_tk, prob,  n_w, experiment_name, project_path, dataset, Nsteps=nsteps, Nbits=nbits)
