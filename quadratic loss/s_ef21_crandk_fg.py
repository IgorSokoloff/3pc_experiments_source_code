"""
EF21-cRandk with full gradient
"""
...
def ef21_crandk_fg_estimator(A, x, b,  k, g_ar, n_workers, rs, probs):
    #updated version using singe loop only
    grad = np.zeros(x.shape[0], dtype=np.float64)
    for i in range(n_workers):
        grad_i = quadratic_grad_i(x, A[i], b[i]).copy()
        grad = grad + grad_i
        delta_i = grad_i - g_ar[i]
        g_ar[i] = g_ar[i] + rand_k_compressor(delta_i, k, rs, probs)
    size_value_sent = 64*1e-6
    grad = grad/n_workers
    return g_ar, size_value_sent, grad

def ef21_crandk_fg(x_0, x_star, f_star, A, b, stepsize, eps, k, n_workers, experiment_name, project_path, dataset, Nsteps, Nbits):
    ...
    while stopping_criterion(sq_norm_ar[-1], eps, it, Nsteps, bits_od_ar[-1], Nbits):
        x = x - stepsize*g
        g_ar, size_value_sent, grad = ef21_crandk_fg_estimator(A, x, b, k, g_ar, n_workers, rs, probs)
        g = np.mean(g_ar, axis=0)
        ...
        it += 1
        bits_od_ar.append(it*k*size_value_sent)
...
if k_rk == dim:
    step_size_ef21_crandk_fg = (1/L_0)*factor
else:
    al = k_rk/dim
    t = -1 + np.sqrt(1/(1-al))
    theta = 1 - (1 - al)*(1 + t)
    ...
    step_size_ef21_crandk_fg = np.float64((1/(L_0 + Lt*np.sqrt(beta/theta)))*factor)

experiment_name = "ef21-crandk-fg_d{0}_nw{1}_s{2}_{3}x".format(dim, n_w, myrepr(s), myrepr(factor))

print ('------------------------------------------------------------------------------------------------')
start_time = time.time()
ef21_crandk_fg(x_0, x_star, f_star, sX, y, step_size_ef21_crandk_fg, eps,  k_rk, n_w, experiment_name, project_path, dataset, Nsteps=nsteps, Nbits=nbits)
