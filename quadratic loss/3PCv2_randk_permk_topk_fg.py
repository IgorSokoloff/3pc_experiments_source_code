"""
3PCv2-RandK-PermK-Topk with full gradient
"""
...
def anna_fg_estimator(A, x, b,  k_tk, k_rk, k_q, g_ar, grads_prev,  n_workers, rs, rs_perm, probs):
    ...
    for i in range(n_workers):
        ...
        grads[i] = grad_i.copy()
        grad = grad + grad_i
        q_dif = permk_compressor(grads[i] - grads_prev[i], perm, k_q, i, n_workers)
        b_i = g_ar[i] + rand_k_compressor(q_dif, k_rk, rs, probs)
        delta_i = grad_i - b_i
...
def anna_fg(x_0, x_star, f_star, A, b, stepsize, eps, k_tk, k_rk, k_q, n_workers, experiment_name, project_path, dataset, Nsteps, Nbits):
    ...
    while stopping_criterion(sq_norm_ar[-1], eps, it, Nsteps, bits_od_ar[-1], Nbits):
        x = x - stepsize*g
        g_ar, size_value_sent, grads, grad = anna_fg_estimator(A, x, b, k_tk, k_rk,k_q, g_ar, grads, n_workers, rs, rs_perm, probs)
        g = np.mean(g_ar, axis=0)
...
omega_rk = dim/k_rk - 1
omega_perm = n_w - 1
omega = (omega_rk + 1)*(omega_perm + 1)-1
theta = al_tk
beta = (1 - al_tk)*omega
Lt = np.sqrt(np.mean(L**2))
step_size_anna_fg = np.float64((1/(L_0 + Lt*np.sqrt(beta/theta)))*factor)

experiment_name = "anna-randk-permk-topk-fg_d{0}_nw{1}_s{2}_{3}x".format(dim, n_w, myrepr(s), myrepr(factor))

print ('------------------------------------------------------------------------------------------------')
start_time = time.time()
anna_fg(x_0, x_star, f_star, sX, y, step_size_anna_fg, eps,  k_tk, k_rk, k_q, n_w, experiment_name, project_path, dataset, Nsteps=nsteps, Nbits=nbits)
